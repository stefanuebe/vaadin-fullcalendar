...
| Start / End | `setStart` / `setEnd` | `LocalDate` or `LocalDateTime`; `LocalDate` implies all-day |
...
| Exclusion dates | `setExdate(String)` | Comma-separated ISO dates excluded from an RRule recurrence |
...
- `setButtonHints(Map<String, String>)` — set `aria-label` on toolbar buttons
...
| `EntryClickedEvent` | User clicks an entry (including keyboard activation when `eventInteractive` is true) |
...
calendar.nextYear();       // same interval, next year
calendar.previousYear();   // same interval, previous year
calendar.incrementDate("P1W");  // move forward by a custom duration
calendar.updateSize();     // force recalculation of dimensions (use after showing a hidden container)
```
...
- Resource grouping by field value (`setResourceGroupField`) with customisable group headers
