...
**As a** Vaadin application developer, **I want to** apply the Vaadin theme variant to the calendar **so that** the calendar visually matches other Vaadin components (Lumo styling).

> **Note (v6 backport):** This branch targets Vaadin 24.10 with the Lumo theme only. Aura is not available.
...
- When the Vaadin theme variant is applied, the calendar adopts Vaadin Lumo colors, fonts, and spacing
...
| BR-02 | The variant applies `full-calendar-theme-lumo.css` via `@CssImport` |
