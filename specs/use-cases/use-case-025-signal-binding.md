...
| OQ-05 | **Property-level signal binding on Entry** | **Won't do:** Considered `entry.bindTitle(Signal<String>)` etc. but unlikely to be a real-world use case. Entry modifications are better handled via `ValueSignal.modify()` on the ListSignal entry. |
