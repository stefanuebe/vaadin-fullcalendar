...
| OQ-05 | **Property-level signal binding on Entry** | **Future work:** Entry properties (title, color, start, end, etc.) could be bindable via `entry.bindTitle(Signal<String>)`, `entry.bindColor(Signal<String>)`, etc. — following the standard Vaadin `bindXyz` pattern. `setTitle()` would throw `BindingActiveException` when bound. Internally, each bound property registers an effect that calls `refreshItem()` on the calendar. Entry already has a calendar back-reference (`getCalendar()`). This would enable direct two-way binding between Vaadin form fields and calendar entries. Requires adding `SignalPropertySupport` per bindable property on Entry — significant API expansion. |
